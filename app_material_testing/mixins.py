...
class TestingTeamAccessMixin(TestingAccessMixin):
    ...
    由 TestingTaskAccessMixin.check_object_permission 通过 user_has_access 调用。
    """
...
class TestingTaskAccessMixin(TestingAccessMixin):
    """测试任务列表/详情页权限 — 研发按项目负责人隔离，测试中心人员跳过 L4/L5。
    ...
    def _isolation_field_name(self, model):
        """列表过滤的 owner 关联字段：按项目负责人（TestingTask → production_order__project__manager）。"""
