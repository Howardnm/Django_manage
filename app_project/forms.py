...
class ProjectMemberForm(TablerFormMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'workload_share': forms.NumberInput(attrs={'step': '1', 'min': '0', 'max': '100'}),
        }
    ...
    def clean_workload_share(self):
        ...
        if workload is not None:
            ...
            if workload != int(workload):
                raise forms.ValidationError("工作量占比只允许填写整数")
            if workload < 0 or workload > 100:
                raise forms.ValidationError("工作量占比需在 0 ~ 100 之间")
            ...
            if self.project:
                ...
                # 校验：已有总和 + 准备录入的权重（百分比 0-100）
                if total_sum + workload > 100:
                    available = 100 - total_sum
                    raise forms.ValidationError(
                        f"总工作量不能超过 100%。当前剩余可用配额仅剩: {available:.0f}%"
                    )
...
class ProjectSalesMemberForm(TablerFormMixin, forms.ModelForm):
    ...
    def clean_workload_share(self):
        ...
        if workload is not None:
            # 只允许 0~100 的整数
            if workload != int(workload):
                raise forms.ValidationError("销售工作量占比只允许填写整数")
            if workload < 0 or workload > 100:
                raise forms.ValidationError("销售工作量占比需在 0 ~ 100 之间")
            ...
            if self.project:
                ...
                if total_sum + workload > 100:
                    available = 100 - total_sum
                    raise forms.ValidationError(
                        f"销售总工作量不能超过 100%。当前剩余可用配额仅剩: {available:.0f}%"
                    )
