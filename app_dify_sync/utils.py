...
def _format_project(instance: Project):
    ...
    info_lines = [
        f"- **项目ID**: {instance.pk}",
        f"- **负责人**: {instance.manager.username}",
        f"- **当前阶段**: {instance.get_current_stage_display()}",
    ]
    if repo:
        if repo.customer:
            info_lines.append(f"- **客户**: {repo.customer.company_name}")
        if repo.oem:
            info_lines.append(f"- **主机厂**: {repo.oem.name}")
        if repo.material:
            info_lines.append(f"- **选用材料**: {repo.material.grade_name}")
...
def _format_material_library(instance: MaterialLibrary):
    ...
    related_formulas = instance.formulas.all()
    formulas_string = ""
    if related_formulas:
        formula_lines = [f"- {f.code}: {f.name}" for f in related_formulas]
        formulas_string = "\n".join(formula_lines)
    else:
        formulas_string = "无"
...
def _format_lab_formula(instance: LabFormula):
    bom_lines = []
    for line in instance.bom_lines.select_related('raw_material').all():
        bom_lines.append(f"- {line.raw_material.name} ({line.raw_material.model_name}): {line.percentage}%")
    bom_string = "\n".join(bom_lines)
